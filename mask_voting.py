...
# ---------- Primary strict rule (original) ----------
COVERAGE_REQ = 0.85
AREA_LOW, AREA_HIGH = 0.85, 2.0          # ratio = box_area / mask_area

# ---------- Fallback 1: single-mask edge-hug ----------
...
def _addweighted_gpu(img_bgr: np.ndarray, mask_any_t: torch.Tensor, alpha=0.45) -> np.ndarray:
    ...
    # edges (use Kornia Canny if available; otherwise Sobel fallback — all on GPU)
    if _HAS_KORNIA:
        ...
        # Build grayscale as (1,1,H,W) directly (avoid empty slices)
        # img_t is BGR in [0,1], shape (1,3,H,W)
        gray = (
            0.114 * img_t[:, 0:1, :, :] +   # B
            0.587 * img_t[:, 1:2, :, :] +   # G  <-- FIXED: 1:2 (not 1:1)
            0.299 * img_t[:, 2:3, :, :]     # R
        )  # (1,1,H,W)
    ...
    # paint edges red
    out[:, 2, :, :] = torch.where(edges[:, 0] > 0, torch.tensor(1.0, device=DEVICE, dtype=out.dtype), out[:, 2, :, :])
    out[:, 1, :, :] = torch.where(edges[:, 0] > 0, torch.tensor(0.0, device=DEVICE, dtype=out.dtype), out[:, 1, :, :])
    out[:, 0, :, :] = torch.where(edges[:, 0] > 0, torch.tensor(0.0, device=DEVICE, dtype=out.dtype), out[:, 0, :, :])

    out = (out.clamp(0, 1) * 255.0).byte().squeeze(0).permute(1, 2, 0).contiguous().cpu().numpy()
    return out


# -------------------- Main pipeline (same logic) --------------------
def overlay_masks(img_bgr, masks_t, alpha=0.45):
    """masks_t: list of (1,1,H,W) float32 tensors on device."""
    if not masks_t:
        return img_bgr
    # union of masks
    union = torch.zeros((1, 1, img_bgr.shape[0], img_bgr.shape[1]), dtype=torch.float32, device=DEVICE)
    for t in masks_t:
...
def main():
    ...
    ANN_DIR = OUT_DIR / "annotated"; ANN_DIR.mkdir(exist_ok=True, parents=True)
    LABELED_DIR = OUT_DIR / "masks_labeled"; LABELED_DIR.mkdir(exist_ok=True, parents=True)

    images = sorted(RENDERS.glob("view_*.png"))
    if not images:
        print(f"[ERR] No images in {RENDERS}/view_*.png"); return
    ...
    for img_path in images:
        ...
        if not dets_path.exists():
            print(f"[WARN] missing dets: {dets_path}, skipping"); continue
        dets = np.load(dets_path, allow_pickle=True).tolist()

        masks_t = load_fastsam_masks(ROOT, stem, H, W, device=DEVICE)
        if not masks_t:
            print(f"[INFO] no masks for {stem}, skipping voting"); continue
        ...
        for det in dets:
            ...
            if best is not None:
                i, mb = best
                if label != "unknown":
                    # save mask to numpy for output parity
                    np_mask = (mb[0, 0].detach().cpu().numpy() > 0.5).astype(np.uint8)
                    labeled.append({"mask": np_mask, "label": label, "matched_box": box, "score": score})
            ...
            candidate_ids = [i for i, m in enumerate(masks_t)
        ...
        out_labeled = (OUT_DIR / "masks_labeled" / f"masks_{stem}.npy")
        ...
        cv2.imwrite(str(OUT_DIR / "annotated" / f"{stem}_matched_masks.jpg"), annotated)
