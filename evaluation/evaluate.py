...
def remap_predictions(pred_labels: np.ndarray,
    ...
    """
    Remap prediction IDs to SUM-Parts IDs.
    
    Args:
        pred_labels: (F,) array with prediction IDs (0, 1, 2, 3, ...)
        pred_to_ram: Mapping from pred ID to RAM++ label
        ram_to_sumparts: Mapping from RAM++ label to SUM-Parts ID
        verbose: Print remapping details
    
    Returns:
        remapped: (F,) array with SUM-Parts IDs (0-12)
    """
    if verbose:
        print(f"\n[REMAPPING]")
        print(f"[INFO] Step 1: Prediction ID → RAM++ Label")
    ...
    if verbose:
        ...
        for pred_id in sorted(np.unique(pred_labels))[:20]:
            count = (pred_labels == pred_id).sum()
            pct = 100 * count / len(pred_labels)
            ram_label = pred_to_ram.get(pred_id, "unknown")
            sumparts_id = pred_to_sumparts.get(pred_id, -1)
            sumparts_name = SUMPARTS_CLASSES.get(sumparts_id, "unmapped") if sumparts_id >= 0 else "unmapped"
            print(f"    ID {pred_id:3d} ('{ram_label:15s}') → {sumparts_name:20s}: {count:6d} faces ({pct:5.2f}%)")
...
def compute_metrics(gt_labels: np.ndarray,
    ...
    if ignore_class >= 0:
        ...
        if verbose:
            print(f"[INFO] Evaluating on {len(gt)}/{len(gt_labels)} faces ({100*len(gt)/len(gt_labels):.1f}%)")
    else:
        gt = gt_labels
        pred = pred_labels
    ...
    if verbose:
        ...
        gt_classes_present = set(np.unique(gt_labels[valid_mask]) if ignore_class >= 0 else np.unique(gt_labels))
        ...
        if total_faces > 0:
            acc = 100 * total_correct / total_faces
            print(f"\n  Summary: {total_correct:6d}/{total_faces:6d} faces correct ({acc:.2f}%)")
    ...
    return {
        ...
        "num_faces_evaluated": int(len(gt)),
    }
...
def evaluate(gt_ply: str,
    ...
    print(f"  Faces Evaluated:          {results['num_faces_evaluated']}")
    
    print(f"\n{'='*70}")
    print(f"Per-Class Results:")
    print(f"{'='*70}")
    print(f"{'Class':<25} {'IoU':>10} {'Accuracy':>10}")
    print(f"{'-'*70}")
    ...
    for class_name, iou in sorted_classes:
        ...
        print(f"{class_name:<25} {iou:>10.4f} {acc:>10.4f}")
